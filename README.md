...
```bash
mkvirtualenv aiction_reaiction -p python3.11
...
Create a database named `aiction_reaiction`.

```
createdb aiction_reaiction
```
...
```bash
celery -A aiction_reaiction worker -l INFO
```

Or with celery beat (for scheduled tasks):

```bash
celery -A aiction_reaiction worker -l INFO -B
```
